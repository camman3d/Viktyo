...
object ActivityStream {
  ...
  object generate {
     ...
     */
    object general {
      def comment(user: User, comment: String, target: Long): ActivityStream = {
        val text = Text(NotAssigned, comment).save
        ActivityStream(NotAssigned, new Date().getTime, user, "comment", text.objId, target)
      }

      def imagePost(user: User, image: Image, target: Long): ActivityStream =
        ActivityStream(NotAssigned, new Date().getTime, user, "imagePost", image.objId, target)

      def statusUpdate(user: User, update: String, target: Long): ActivityStream = {
     ...
     */
    object users {
      def profilePicture(user: User, image: Image): ActivityStream =
        ActivityStream(NotAssigned, new Date().getTime, user, "profilePicture", image.objId, user.objId)

      def follow(user: User, otherUser: User): ActivityStream =
     ...
     */
    object networks {
      def join(user: User, network: Network): ActivityStream =
     ...
     */
    object postings {
      def create(user: User, posting: Posting): ActivityStream =
        ActivityStream(NotAssigned, new Date().getTime, user, "create", posting.objId, posting.objId)

      def updatedCover(user: User, posting: Posting, image: Image): ActivityStream =
        ActivityStream(NotAssigned, new Date().getTime, user, "updatedCover", image.objId, posting.objId)

      def follow(user: User, posting: Posting): ActivityStream =
        ActivityStream(NotAssigned, new Date().getTime, user, "follow", posting.objId, posting.objId)

      def favorite(user: User, posting: Posting): ActivityStream =
