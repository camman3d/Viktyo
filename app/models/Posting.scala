  ...
  def save: Posting = {
    DB.withConnection { implicit connection =>
      if (this.id.isDefined) {
        ...
        // Return the posting
        Posting(Id(id.get), this.name, this.posted, this.poster, newLocation, newProperties, this.objId)
      }
    }
  }
...
object Posting {
  val simple = {
    get[Pk[Long]]("posting.id") ~
      ...
      case id~name~posted~poster~location~objId => Posting(id, name, posted, User.findById(poster).get, Location.findById(location).get, Property.listByObjId(objId), objId)
    }
  }
